...
from sky_executor.grid_factory.factory.grid_factory_env.Utils.single_step_svg import SingleStepAnimationMonitor
...
def run_policy(gc: GridConfig, save_animation=False):
    ...
    if save_animation:
        ...
        env = SingleStepAnimationMonitor(env)
    while True:
        obs, info = env.reset()
        print(f"初始观察{obs}")
        while True:
...
if __name__ == '__main__':
    print(233)
    gc = GridConfig(num_agents=20, size=8, obs_radius=4, seed=42, max_episode_steps=64, on_target='finish')
    print(2333)
    results_generator = run_policy(gc, save_animation=True)
    try:
        metrics = next(results_generator)
        print(f"第一回合指标: {metrics}")
        # 如果需要运行多个回合，可以继续调用next
        # metrics = next(results_generator)
        # print(f"第二回合指标: {metrics}")
