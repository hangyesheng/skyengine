...
from sky_executor.grid_factory.factory.grid_factory_env.Utils.pic_drawer import draw_svg,draw_svg_with_machine
def sky_test_new_draw_method():
    """测试网格工厂环境"""
    print("=== 测试新的绘图方法 ===")
...
def sky_test_machine_draw_method():
    ...
    # 测试环境重置
    obs, info = env.reset(seed=42)
    res=draw_svg_with_machine(env.pogema_env,env.machines)
    with open("temp.svg", "w") as f:
        f.write(res)
