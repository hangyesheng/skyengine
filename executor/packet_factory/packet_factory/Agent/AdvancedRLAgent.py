            ...
            if path is None:
                ...
                if os.path.exists(default_path):
                    ...
                    if file_size == 0:
                        ...
                        LOGGER.info(f"[AdvancedRLAgent] 检测到默认模型文件为空，创建新文件：{path}")
                    else:
                        # 文件非空，覆盖原有文件
                        path = default_path
                        LOGGER.info(f"[AdvancedRLAgent] 覆盖现有模型文件：{path}")
                else:
                    # 文件不存在，创建默认文件
                    path = default_path
                    LOGGER.info(f"[AdvancedRLAgent] 创建新模型文件：{path}")
            else:
                # 指定了路径，确保目录存在（仅当 path 包含目录时）
